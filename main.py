...
def train(model, X1, A1, X2, A2, y):
    if not opt.args.pretrain:
        ...
        _, _, _, _,_, _,_, centers1 = clustering(Z1, y)
        _, _, _, _, _, _,_, centers2 = clustering(Z2, y)
    ...
    for epoch in pbar:
        ...
        ari, nmi, fmi, hom, com, ami,acc, y_pred = assignment((Q1[0] + Q2[0]).data, y)

        pbar.set_postfix({'loss': '{0:1.4f}'.format(loss), 'ARI': '{0:1.4f}'.format(ari), 'NMI': '{0:1.4f}'.format(nmi),
                          'FMI': '{0:1.4f}'.format(fmi), 'HOM': '{0:1.4f}'.format(hom),
                          'COM': '{0:1.4f}'.format(com), 'AMI': '{0:1.4f}'.format(ami) ,'ACC': '{0:1.4f}'.format(acc)})
...
if __name__ == '__main__':
    ...
    ae1 = AE(
        ae_n_enc_1=opt.args.ae_n_enc_1, ae_n_enc_2=opt.args.ae_n_enc_2,ae_n_enc_3=opt.args.ae_n_enc_3,
    ...
    gae1 = IGAE(
        ...
        n_input=opt.args.n_d1).to(opt.args.device)

    gae2 = IGAE(
        gae_n_enc_1=opt.args.gae_n_enc_1, gae_n_enc_2=opt.args.gae_n_enc_2,gae_n_enc_3=opt.args.gae_n_enc_3,
        gae_n_dec_1=opt.args.gae_n_dec_1, gae_n_dec_2=opt.args.gae_n_dec_2,gae_n_dec_3=opt.args.gae_n_dec_3,
        n_input=opt.args.n_d2).to(opt.args.device)
    ...
    if opt.args.pretrain:
        ...
        model = scTGIC(ae1, ae2, gae1, gae2,  n_node=Xr.shape[0]).to(opt.args.device)
