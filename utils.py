...
def clustering(Z, y):
    ...
    ari, nmi, fmi,  hom, com, ami ,acc= eva(y, cluster_id, show_details=True)

    return ari, nmi, fmi,  hom, com, ami,acc ,model.cluster_centers_


def assignment(Q, y):
    y_pred = torch.argmax(Q, dim=1).data.cpu().numpy()
    ari, nmi, fmi,  hom, com, ami ,acc= eva(y, y_pred, show_details=False)
    return ari, nmi, fmi,  hom, com, ami,acc, y_pred
...
def eva(y_true, y_pred, show_details=True):
    ...
    if show_details:
        print("\n", "ARI: {:.4f},".format(ari), "NMI: {:.4f},".format(nmi), "FMI: {:.4f}".format(fmi),
             "HOM: {:.4f},".format(hom), "COM: {:.4f}".format(com),
              "AMI: {:.4f}".format(ami),"ACC: {:.4f},".format(acc),)

    return ari, nmi, fmi, hom, com, ami, acc
